...
<main>
    ...
    <div class="container-xl px-4 mt-n10">
        <div class="card">
            ...
            <div class="card-body">
                <!-- Contents Here -->
                <a class="btn btn-primary btn-sm float-end mb-2" href="<?= base_url('auditsystem/wp/viewpdfc1/')?><?= $code?>/<?= $mtID?>/<?= $cID?>/<?= $wpID?>" target="_blank" title="View"><i class="fas fa-eye"></i> View Document</a>
                <hr style="color: #7752FE;" class="m-5">
                <div class="m-5">
                    <h4>Client Acceptance or Continuance Form</h4>
                    <h6>This form must be completed by the A.E.P. before any work is undertaken on the file.</h6>
                    ...
                    <form action="<?= base_url()?>auditsystem/wp/savevalues/c1/saveac1/<?= $code?>/<?= $mtID?>/<?= $cID?>/<?= $wpID?>/<?= $name?>" method="post">
                    ...
                    <form action="<?= base_url()?>auditsystem/wp/savevalues/c1/saveac1eqr/<?= $code?>/<?= $mtID?>/<?= $cID?>/<?= $wpID?>/<?= $name?>" method="post">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <label class="mb-1" for="eqr1">It is necessary for an EQR to be performed and this will be performed by:</label>
                                <input type="text" class="form-control border-dark" id="eqr1" name="eqr1" value="<?= $eqr['eqr1']?>">
                                <label class="mb-1" for="eqr2">Where the EQR is undertaken by an external reviewer the name of the organisation which they work for</label>
                                <input type="text" class="form-control border-dark" id="eqr2" name="eqr2" value="<?= $eqr['eqr2']?>">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="mb-1" for="eqrr">REASON FOR EQR (If an EQR review was performed in the previous period, but is not being performed in the current period, this decision must also be justified.)</label>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                <label class="mb-1" for="hcc">Having completed the checklist</label>
                                ...
                                <label class="mb-1" for="iio">consider that there are any perceived threats to our independence, integrity and objectivity, and believe that we</label>
                                ...
                                <label class="mb-1" for="iio">this appointment.</label>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-success m-1 float-end btn-sm"><i class="fas fa-file-alt m-1"></i>Save</button>
                    </form>
