...
$html = '';
    ...
    */
    ...
    // Read the PDF file
    $imagick->readImage(base_url('uploads/pdf/wp/'.$fID.'/'.$wpID.'/doc2.pdf'));
    // Optionally, set the resolution (density) to improve quality
    ...
    foreach ($imagick as $page) {
        ...
        if ($width > $height) {
            // Rotate the image if needed (90 degrees)
            //$page->rotateImage(new ImagickPixel(),);
            // Set the page orientation to landscape in TCPDF
            $pdf->AddPage('L'); // 'L' stands for Landscape orientation
            $pdf->Image('@' . $image_data, 0, 0, $pdf->getPageWidth(), $pdf->getPageHeight(), 'JPG');
        } else {
            // Add a regular portrait page
            $pdf->AddPage();
            $pdf->Image('@' . $image_data, 0, 0, $pdf->getPageWidth(), $pdf->getPageHeight(), 'JPG');
        }
